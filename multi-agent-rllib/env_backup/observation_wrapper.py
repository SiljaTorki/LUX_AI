...
class FlattenNormalizeObservation(gym.ObservationWrapper):
    ...
    def __init__(self, env: gym.Env, multi_agent=False):
        ...
        # Define the observation space based on the multi_agent flag
        single_player_flat_box = Box(low=-1.0, high=1.0, shape=(self.single_player_obs_flat_size,), dtype=np.float32)

        if self.multi_agent:
            # For multi-agent, the space is a Dict of Boxes
            self.observation_space = GymDict({
                "player_0": single_player_flat_box,
                "player_1": single_player_flat_box
            })
        else:
            # For single-agent, the space is a flat Box.
    ...
    def observation(self, obs):
        ...
        for both players.
        """
        if not isinstance(obs, dict) or "player_0" not in obs or "player_1" not in obs:
            print(f"Warning: Unexpected observation structure: {type(obs)}. Returning previous structure or zero array.")
            # Attempt to return the expected structure with zeros.
            if self.multi_agent:
                zero_obs = np.zeros(self.single_player_obs_flat_size, dtype=np.float32)
                return {"player_0": zero_obs, "player_1": zero_obs}
            else:
                return np.zeros(self.observation_space.shape, dtype=self.observation_space.dtype)

        # Create flat boxes per player
        flat_obs_p0 = self._process_single_player_obs(obs["player_0"])
        flat_obs_p1 = self._process_single_player_obs(obs["player_1"])

        if self.multi_agent:
            # Return Dict of flattened observations
            return {
                "player_0": flat_obs_p0.astype(self.observation_space['player_0'].dtype),
                "player_1": flat_obs_p1.astype(self.observation_space['player_1'].dtype)
            }
        else:
            # Concatenate into a single vector
            stacked_obs = np.concatenate([flat_obs_p0, flat_obs_p1])
            if stacked_obs.shape != self.observation_space.shape:
                raise ValueError(f"Shape mismatch: Expected {self.observation_space.shape}, Got {stacked_obs.shape}")
