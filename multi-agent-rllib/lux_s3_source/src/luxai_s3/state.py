...
class EnvState:
    units: UnitState
    """Units in the env with shape (T, N, 3) for T teams, N max units, and 3 features.

    3 features are for position (x, y), and energy
    """
    units_mask: chex.Array
    """Mask of units in the env with shape (T, N) for T teams, N max units"""
    energy_nodes: chex.Array
    """Energy nodes in the env with shape (N, 2) for N max energy nodes, and 2 features.
    ...
    # """Energy field in the env with shape (H, W) for H height, W width. This is generated from other state"""
    
    energy_nodes_mask: chex.Array
    """Mask of energy nodes in the env with shape (N) for N max energy nodes"""
    relic_nodes: chex.Array
    """Relic nodes in the env with shape (N, 2) for N max relic nodes, and 2 features.

    2 features are for position (x, y)
    """
    relic_node_configs: chex.Array
    """Relic node configs in the env with shape (N, K, K) for N max relic nodes and a KxK relic configuration"""
    relic_nodes_mask: chex.Array
    """Mask of relic nodes in the env with shape (N, ) for N max relic nodes"""
    relic_nodes_map_weights: chex.Array
    """Map of relic nodes in the env with shape (H, W) for H height, W width. Each element is equal to the 1-indexed id of the relic node. This is generated from other state"""
    
    relic_spawn_schedule: chex.Array
    """Relic spawn schedule in the env with shape (N, ) for N max relic nodes. Elements are the game timestep at which the relic node spawns"""

    map_features: MapTile
    """Map features in the env with shape (W, H, 2) for W width, H height
    """

    sensor_mask: chex.Array
    """Sensor mask in the env with shape (T, H, W) for T teams, H height, W width. This is generated from other state"""

    vision_power_map: chex.Array
    """Vision power map in the env with shape (T, H, W) for T teams, H height, W width. This is generated from other state"""

    team_points: chex.Array
    """Team points in the env with shape (T) for T teams"""
    team_wins: chex.Array
    """Team wins in the env with shape (T) for T teams"""

    steps: int = 0
    """steps taken in the env"""
    match_steps: int = 0
    """steps taken in the current match"""
    
@struct.dataclass
class EnvObs:
    """Partial observation of env"""
    units: UnitState
    """Units in the env with shape (T, N, 3) for T teams, N max units, and 3 features.
